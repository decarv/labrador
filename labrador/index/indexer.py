...
class QdrantIndexer:
    def __init__(self,
                 ...
                 vectorizer: str = 'none'):
        self.client = QdrantClient("192.168.15.195", port=6333)
        self.collection_name = collection_name(model_name, token_type)
        self.model_name = model_name
        self.token_type = token_type

    def create_index(self):
        if self.collection_name in self.client.get_collection():
        ...
        try:
            self.client.create_collection(
                ...
                vectors_config=models.VectorParams(
                    ...
                    distance=models.Distance.COSINE if config.MODELS[model_name] == 'cosine' else models.Distance.DOT,
                    on_disk=True,
                ),
    ...
    def populate_index(self):
        with psycopg2.connect(
            ...
            for i, records in enumerate(self.records_generator(conn)):
                ...
                if i % 10000 == 0:
                    self.client.create_snapshot(self.collection_name)
                    logger.info("Snapshot created.")
                points: list[PointStruct] = [self._create_object(record) for record in records]
                try:
                    self.client.upsert(
                        collection_name=self.collection_name,
                        points=points,
    ...
    @staticmethod
    def _create_object(record: RealDictRow) -> PointStruct:
